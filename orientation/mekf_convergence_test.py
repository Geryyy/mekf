...
# -------------------------------
# Helper Functions (Reused from mekf_testbench_improved.py)
# -------------------------------
...
def run_convergence_test(num_steps=10000, dt=0.01, noise_level=0.05):
    ...
    output_dir = "mekf_convergence_results"
    ...
    # Compute error slopes (last 50% of data to avoid initial transients)
    ...
    for w in range(num_windows):
        ...
        for i in range(3):
            correlations[w, i] = np.corrcoef(
                ground_truth_omega[start:end, i],
                filter_omega[start:end, i]
            )[0, 1]

    # Log results
    log_file = open(f"{output_dir}/convergence_log.txt", "w")
    log_file.write("MEKF Convergence Test Results\n")
    ...
    plt.tight_layout()
    plt.savefig(f"{output_dir}/convergence_results_noise_{noise_level:.2f}.png")
    plt.close()

    np.savetxt(f"{output_dir}/convergence_data_noise_{noise_level:.2f}.txt",
